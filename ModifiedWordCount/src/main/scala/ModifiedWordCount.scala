...
object ModifiedWordCount extends Configured with Tool {
  val IN_PATH_PARAM: String = "modifiedwordcount.input"
  val OUT_PATH_PARAM: String = "modifiedwordcount.output"

  override def run(args: Array[String]): Int = {
    val job: Job = Job.getInstance(getConf, "Modified Word Count")
...
class TokenizerMapper extends Mapper[AnyRef, Text, Text, IntWritable] {
  ...
  override def map(key: AnyRef,
    ...
    for (word: String <- value.toString.toLowerCase().split("\\s")) {
      val clearWord: String = word.toUpperCase().replaceAll("\\p{Punct}|(\uD83D\uDC47)", "")
      if (clearWord != "") {
        text.set(clearWord)
